...
class UncertaintyAnalysis:
    ...
    def plot_corner_with_marginals(
        ...
        Corner + marginals, with:
          - Off-diagonal: 2D KDE shown via single-color continuous alpha (no colormap),
            plus 68%/95% HPD contours. Association metric printed in the box.
          - Diagonal: weighted hist + KDE + Gaussian; shows HPD68 (smallest contiguous)
            as the constraint metric (most defensible “error bar”).
        ...
        def _weighted_quantile(v, qs, ww):
            v = np.asarray(v, float); qs = np.asarray(qs, float)
            idx = np.argsort(v); vv = v[idx]; ww = np.asarray(ww)[idx]
            c = np.cumsum(ww); c /= c[-1]
            return np.interp(qs, c, vv)
        ...
        def _kde_1d(vals, weights, xs):
            try:
                ...
                N = min(max(1000, 5*len(vals)), 5000)
                idx = rng.choice(np.arange(len(vals)), size=N, replace=True, p=weights/np.sum(weights))
        ...
        def _kde_2d(x, y, weights, Xg, Yg):
            ...
            try:
                ...
                N = min(max(1000, 5*len(x)), 5000)
                idx = rng.choice(np.arange(len(x)), size=N, replace=True, p=weights/np.sum(weights))
                kde = gaussian_kde(np.vstack([x[idx], y[idx]]))
            Z = kde(np.vstack([Xg.ravel(), Yg.ravel()])).reshape(Xg.shape)
            # thresholds for enclosed-mass levels (e.g., 68%, 95%)
            vals = Z.ravel()
            order = np.argsort(vals)[::-1]
            mass = np.cumsum(vals[order]); mass /= mass[-1]
            thr68 = vals[order[np.searchsorted(mass, 0.68, side='left')]]
            thr95 = vals[order[np.searchsorted(mass, 0.95, side='left')]]
            return Z, (thr68, thr95)
        ...
        def _hpd_1d(vals, weights, mass=0.68, grid=1024):
            """Smallest contiguous interval covering `mass` of the KDE posterior."""
            ...
            dens = _kde_1d(v, weights, xs)
            dx = xs[1] - xs[0]
            pmass = dens * dx
            pmass /= pmass.sum()
            c = np.cumsum(pmass)
            # two-pointer window for minimal width with >= mass
            i = 0
            best = (np.inf, 0, 0)
            for j in range(grid):
                while c[j] - (c[i-1] if i > 0 else 0.0) >= mass:
                    width = xs[j] - xs[i]
                    if width < best[0]:
                        best = (width, i, j)
                    i += 1
                    if i > j: break
            i0, j0 = best[1], best[2]
            hpd_lo, hpd_hi = xs[i0], xs[j0]
            med = _weighted_quantile(v, [0.5], weights)[0]
            return hpd_lo, med, hpd_hi
        ...
        for i, pi in enumerate(params):
            ...
            for j, pj in enumerate(params):
                ...
                if i == j:
                    # ----- diagonal: 1-D posterior with HPD68 -----
                    v = xi[np.isfinite(xi)]
                    if v.size == 0: ax.axis('off'); continue
                    xmin, xmax = v.min(), v.max(); pad = 0.08*(xmax - xmin + 1e-12)
                    xs = np.linspace(xmin - pad, xmax + pad, 512)

                    # weighted hist
                    ax.hist(v, bins=bins, density=True, color='lightgray', edgecolor='black', alpha=0.6)

                    # KDE + Gaussian
                    dens = _kde_1d(v, w, xs)
                    ax.plot(xs, dens, lw=2, color='C0')
                    mu = float(np.average(v, weights=w))
                    sig = float(np.sqrt(np.average((v-mu)**2, weights=w)))
                    if np.isfinite(sig) and sig > 0:
                        g = (1.0/(np.sqrt(2*np.pi)*sig))*np.exp(-0.5*((xs-mu)/sig)**2)
                        ax.plot(xs, g, ls='--', lw=1.2, color='C1')

                    # HPD68
                    hpd_lo, med, hpd_hi = _hpd_1d(v, w, mass=hpd_mass)
                    ax.axvline(med, color='red', ls='--', lw=1.6)
                    ax.axvline(hpd_lo, color='orange', ls=':', lw=1.4)
                    ax.axvline(hpd_hi, color='orange', ls=':', lw=1.4)

                    # annotation (most defensible error)
                    ax.text(0.98, 0.96, f'HPD68: {med:.3g} −{(med-hpd_lo):.3g}, +{(hpd_hi-med):.3g}',
                            transform=ax.transAxes, ha='right', va='top',
                            fontsize=12, bbox=dict(facecolor='white', edgecolor='0.7', boxstyle='round,pad=0.2'))
                    ...
                    # ----- lower triangle: smooth colormap KDE + faint points -----
                    x = np.asarray(top[pj].values, float); y = xi
                    nx = ny = 180
                    xg = np.linspace(np.nanmin(x), np.nanmax(x), nx)
                    yg = np.linspace(np.nanmin(y), np.nanmax(y), ny)
                    Xg, Yg = np.meshgrid(xg, yg)
                    Z, (thr68, thr95) = _kde_2d(x, y, w, Xg, Yg)

                    # smooth background (like your fitness heatmap)
                    ax.imshow(
                        Z,
                        extent=(xg.min(), xg.max(), yg.min(), yg.max()),
                        origin='lower',
                        interpolation='bilinear',   # smooth
                        cmap='turbo',               # change to 'viridis' if you prefer
                        aspect='auto'
                    )

                    # 68/95% HPD contours — MUST be increasing
                    levels = np.sort([thr68, thr95])
                    if np.isclose(levels[0], levels[1]):  # numerical guard
                        levels[0] *= 0.999
                    ax.contour(Xg, Yg, Z, levels=levels, colors='k', linewidths=1.0)

                    # faint, small markers
                    ax.scatter(x, y, s=4, c='k', alpha=0.12, linewidths=0)
                    ...
                    # tidy labels (no gaps retained elsewhere via wspace=0, hspace=0)
                    if j == 0:
    ...
    def choose_cutoff_lognorm_mixture(
            self, bins=100, kde_points=1024, em_max_iter=200, tol=1e-6
        ):
        """
        ONE method: 2-component lognormal mixture (EM on y=log(loss)).
        Cutoff = intersection where posterior responsibilities tie (0.5/0.5).
        Writes two plots and returns cutoff & realized keep fraction.
        """
        ...
        # ----- EM for 2-Gaussian mixture in y-space -----
        N = y.size
        # init by quantiles
        m1, m2 = np.quantile(y, [0.2, 0.8])
        s = float(np.std(y)) if np.std(y) > 1e-6 else 0.1
        mu = np.array([m1, m2], float)
        sig = np.array([s, s], float)
        pi = np.array([0.5, 0.5], float)

        for _ in range(em_max_iter):
            # E-step
            p1 = pi[0] * norm.pdf(y, mu[0], sig[0] + 1e-12)
            p2 = pi[1] * norm.pdf(y, mu[1], sig[1] + 1e-12)
            denom = p1 + p2 + 1e-300
            r1 = p1 / denom
            r2 = 1.0 - r1

            # M-step
            N1, N2 = np.sum(r1), np.sum(r2)
            mu_new = np.array([np.sum(r1*y)/max(N1,1e-12), np.sum(r2*y)/max(N2,1e-12)])
            sig_new = np.array([
                np.sqrt(np.sum(r1*(y-mu_new[0])**2)/max(N1,1e-12)),
                np.sqrt(np.sum(r2*(y-mu_new[1])**2)/max(N2,1e-12))
            ])
            pi_new = np.array([N1/N, N2/N])

            if np.allclose(mu, mu_new, rtol=0, atol=tol) and np.allclose(sig, sig_new, rtol=0, atol=tol):
                mu, sig, pi = mu_new, np.maximum(sig_new, 1e-6), pi_new
                break
            mu, sig, pi = mu_new, np.maximum(sig_new, 1e-6), pi_new

        # order components so comp0 is elite (lower mean => lower loss)
        order = np.argsort(mu)
        mu, sig, pi = mu[order], sig[order], pi[order]

        # ----- intersection in y-space where pi0 N0 = pi1 N1  (quadratic/linear) -----
        A = 0.5*(1.0/sig[1]**2 - 1.0/sig[0]**2)
        B = (mu[0]/sig[0]**2 - mu[1]/sig[1]**2)
        D = 0.5*(mu[1]**2/sig[1]**2 - mu[0]**2/sig[0]**2)
        const = np.log((pi[1]/sig[1])/(pi[0]/sig[0]))  # RHS
        C = D - const
        ...
        if abs(A) < 1e-12:
            ...
            # numerical guard
            disc = max(disc, 0.0)
            roots = np.sort(( -B + np.array([-1,1])*np.sqrt(disc) ) / (2*A))
            # pick root between the two means if available; else the nearer
            if roots[0] <= mu[1] and roots[1] >= mu[0]:
                y_cut = roots[0] if (mu[0] <= roots[0] <= mu[1]) else roots[1]
            else:
                y_cut = roots[np.argmin(np.abs(roots - np.mean(mu)))]

        loss_cutoff = float(np.exp(y_cut))
        frac = float(np.mean(L <= loss_cutoff))
        pct = 100.0 * frac

        # ----- Plot 1: distribution with mixture & cutoff -----
        xs = np.linspace(L.min(), L.max(), kde_points)
        kde = gaussian_kde(L)(xs)
        # lognormal pdfs
        def lnpdf(x, m, s):  # x>0
            return (1.0/(x*(s*np.sqrt(2*np.pi)))) * np.exp(-0.5*((np.log(x+eps)-m)/s)**2)
        mix = pi[0]*lnpdf(xs, mu[0], sig[0]) + pi[1]*lnpdf(xs, mu[1], sig[1])

        fig, ax = plt.subplots(figsize=(8,5))
        ax.hist(L, bins=bins, density=True, alpha=0.6, color='lightsteelblue', edgecolor='black', label='Histogram')
        ax.plot(xs, kde, color='C0', lw=2, label='KDE')
        ax.plot(xs, pi[0]*lnpdf(xs, mu[0], sig[0]), color='C1', ls='--', lw=1.6, label=f'elite comp (w={pi[0]:.2f})')
        ax.plot(xs, pi[1]*lnpdf(xs, mu[1], sig[1]), color='C3', ls='--', lw=1.6, label=f'bulk comp (w={pi[1]:.2f})')
        ax.plot(xs, mix, color='k', lw=1.2, alpha=0.8, label='mixture')
        ax.axvline(loss_cutoff, color='C2', lw=2.2, label=f'cutoff: {loss_cutoff:.4g}  (keep≈{pct:.1f}%)')
        ...
        # ----- Plot 2: cumulative population vs loss -----
        ...
        # small audit file
        with open(os.path.join(self.output_path, 'uncertainty', 'loss_cutoff.txt'), 'w') as f:
            f.write("method: lognormal_mixture_intersection\n")
            f.write(f"mu: {mu.tolist()}\n")
            f.write(f"sig: {sig.tolist()}\n")
            f.write(f"pi: {pi.tolist()}\n")
            f.write(f"loss_cutoff: {loss_cutoff:.8g}\n")
            f.write(f"fraction_kept: {frac:.6f}\n")
            f.write(f"percentile_kept: {pct:.4f}\n")

        return {
            'loss_cutoff': loss_cutoff,
            'fraction': frac,
            'percentile': pct,
            'paths': {'dist': dist_path, 'cdf': cdf_path}
        }
    ...
    def generate_comprehensive_uncertainty_report(self):
        ...
        # NEW: plotting functions that correspond to the paper bullets (2) and (3)
        print("Rendering weighted 1D posteriors...")
        path_1d_weighted, _kdes = self.plot_marginalized_posteriors_kde_weighted(percentile=ptile)
...
def main(main_dir):
    """
    Example usage of the uncertainty analysis framework.
    """
    # Initialize analysis (adjust paths as needed)
    results_file = main_dir+ "/simulation_results.csv"  # Update this path
    output_path = main_dir + "/analysis/"
    
    if not os.path.exists(results_file):
        print(f"Error: Results file {results_file} not found!")
        print("Please update the results_file path in the main() function.")
        results_file = main_dir+ "/simulation_results_gen_80.csv"  # Update this path
        output_path = main_dir + "/analysis/"
        ...
        if not os.path.exists(results_file):
            ...
            results_file = main_dir+ "/simulation_results_gen_40.csv"  # Update this path
            output_path = main_dir + "/analysis/"
    ...
    analyzer = UncertaintyAnalysis(results_file, output_path)
    
    # Run comprehensive analysis
    results = analyzer.generate_comprehensive_uncertainty_report()
    
    print("\nAnalysis complete! Check the following files:")
    print(f"- {output_path}analysis/uncertainty/bootstrap_results.txt")
    print(f"- {output_path}analysis/uncertainty/weighted_stats.txt")
    print(f"- {output_path}analysis/uncertainty/marginalized_posteriors.png")
    print(f"- {output_path}analysis/uncertainty/comprehensive_report.txt")
